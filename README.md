...
- [Usage](#usage)
    ...
    - [MerchantCard](#query-merchantcards): View cards saved from previously approved purchases
    - [MerchantInstallment](#query-merchantinstallments): View installments generated by split MerchantPurchases
    - [MerchantPurchase](#create-a-merchantpurchase): Create card purchases and manage them
    - [Split](#query-splits): View the splits generated when a split Invoice is paid
    - [SplitReceiver](#create-splitreceivers): Manage the receivers of an Invoice split
    - [SplitProfile](#create-splitprofile-or-update-it): Configure the payout schedule for your split receivers
...
Stores information about a card used in an approved purchase, so it can be reused in new purchases without a new
MerchantSession. To search for MerchantCards, run:

```ruby
require('starkbank')

cards = StarkBank::MerchantCard.query(limit: 10)
...
To get a single MerchantCard by its id, run:
...
You can query MerchantCard logs to better understand MerchantCard life cycles.

```ruby
require('starkbank')

logs = StarkBank::MerchantCard::Log.query(limit: 50)
...
log = StarkBank::MerchantCard::Log.get('5155165527080960')
...
Represents one installment of a MerchantPurchase, generated automatically by the Stark Bank API when the purchase
is split. To search for MerchantInstallments, run:

```ruby
require('starkbank')

installments = StarkBank::MerchantInstallment.query(limit: 10)
...
To get a single MerchantInstallment by its id, run:
...
You can query MerchantInstallment logs to better understand MerchantInstallment life cycles.

```ruby
require('starkbank')

logs = StarkBank::MerchantInstallment::Log.query(limit: 50)
...
log = StarkBank::MerchantInstallment::Log.get('5155165527080960')
...
When not created through a MerchantSession, a MerchantPurchase requires card and holder data directly:
...
**Note**: Instead of using MerchantPurchase objects, you can also pass each element in dictionary format

## Query MerchantPurchases

To search for MerchantPurchases, run:

```ruby
require('starkbank')

purchases = StarkBank::MerchantPurchase.query(limit: 10)
...
To get a single MerchantPurchase by its id, run:
...
If the purchase is "approved", you may only cancel it by passing status: 'canceled' together with amount: 0. If the
purchase is "confirmed", you may pass status: 'reversed' with a lower amount to debit and reverse the difference,
partially or totally; a partial reversal keeps status "confirmed", while a full reversal moves it to "voided".

```ruby
require('starkbank')

purchase = StarkBank::MerchantPurchase.update('5155165527080960', status: 'reversed', amount: 0)
...
You can query MerchantPurchase logs to better understand MerchantPurchase life cycles.

```ruby
require('starkbank')

logs = StarkBank::MerchantPurchase::Log.query(limit: 50)
...
log = StarkBank::MerchantPurchase::Log.get('5155165527080960')
...
When an Invoice with split rules is paid, the API generates a Split for each receiver. You can get a list
of created Splits given some filters.
...
logs = StarkBank::Split::Log.query(limit: 50)
...
**Note**: Instead of using SplitReceiver objects, you can also pass each element in dictionary format
...
logs = StarkBank::SplitReceiver::Log.query(limit: 50)
...
## Create SplitProfile or update it

When you create a Split, a SplitProfile is automatically created for its receiver. If you haven't created
a Split yet, you can use the 'put' method to create your own SplitProfile and configure the payout schedule
in advance.
...
**Note**: Instead of using SplitProfile objects, you can also pass each element in dictionary format

## Query SplitProfiles

To take a look at the SplitProfiles created to your workspace, just run the following:

```ruby
require('starkbank')

profiles = StarkBank::SplitProfile.query(limit: 10)
...
logs = StarkBank::SplitProfile::Log.query(limit: 50)
...
log = StarkBank::SplitProfile::Log.get('5155165527080960')
