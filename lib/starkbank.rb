# frozen_string_literal: true

require_relative('workspace/workspace')
require_relative('balance/balance')
require_relative('transaction/transaction')
require_relative('invoice/invoice')
require_relative('invoice/log')
require_relative('invoice/rule')
require_relative('invoice/payment')
require_relative('invoice_pull_request/invoicepullrequest')
require_relative('invoice_pull_request/log')
require_relative('invoice_pull_subscription/invoicepullsubscription')
require_relative('invoice_pull_subscription/log')
require_relative('dict_key/dict_key')
require_relative('dynamic_brcode/dynamic_brcode')
require_relative('dynamic_brcode/rule')
require_relative('deposit/deposit')
require_relative('deposit/log')
require_relative('card_method/cardmethod')
require_relative('corporate_balance/corporatebalance')
require_relative('corporate_card/corporatecard')
require_relative('corporate_card/log')
require_relative('corporate_holder/corporateholder')
require_relative('corporate_holder/log')
require_relative('corporate_holder/permission')
require_relative('corporate_invoice/corporateinvoice')
require_relative('corporate_purchase/corporatepurchase')
require_relative('corporate_purchase/log')
require_relative('corporate_rule/corporaterule')
require_relative('corporate_transaction/corporatetransaction')
require_relative('corporate_withdrawal/corporatewithdrawal')
require_relative('merchant_category/merchantcategory')
require_relative('merchant_country/merchantcountry')
require_relative('merchant_session/merchant_session')
require_relative('merchant_session/purchase')
require_relative('merchant_session/allowed_installment')
require_relative('merchant_session/log')
require_relative('merchant_card/merchant_card')
require_relative('merchant_card/log')
require_relative('merchant_installment/merchant_installment')
require_relative('merchant_installment/log')
require_relative('merchant_purchase/merchant_purchase')
require_relative('merchant_purchase/log')
require_relative('split_receiver/split_receiver')
require_relative('split_receiver/log')
require_relative('split_profile/split_profile')
require_relative('split_profile/log')
require_relative('split/split')
require_relative('split/log')
require_relative('brcode_payment/brcode_payment')
require_relative('brcode_payment/rule')
require_relative('brcode_payment/log')
require_relative('boleto/boleto')
require_relative('boleto/log')
require_relative('boleto_holmes/boleto_holmes')
require_relative('boleto_holmes/log')
require_relative('transfer/transfer')
require_relative('transfer/log')
require_relative('transfer/rule')
require_relative('verified_account/verifiedaccount')
require_relative('verified_account/log')
require_relative('verified_transfer/verifiedtransfer')
require_relative('boleto_payment/boleto_payment')
require_relative('boleto_payment/log')
require_relative('utility_payment/utility_payment')
require_relative('utility_payment/log')
require_relative('tax_payment/tax_payment')
require_relative('tax_payment/log')
require_relative('darf_payment/darf_payment')
require_relative('darf_payment/log')
require_relative('webhook/webhook')
require_relative('event/event')
require_relative('event/attempt')
require_relative('payment_request/payment_request')
require_relative('payment_preview/payment_preview')
require_relative('payment_preview/brcode_preview')
require_relative('payment_preview/boleto_preview')
require_relative('payment_preview/tax_preview')
require_relative('payment_preview/utility_preview')
require_relative('institution/institution')
require_relative('request/request')

# SDK to facilitate Ruby integrations with Stark Bank
module StarkBank

  API_VERSION = 'v2'
  SDK_VERSION = '2.15.0'
  HOST = "bank"
  public_constant :API_VERSION, :SDK_VERSION, :HOST;

  @user = nil
  @language = 'en-US'
  @timeout = 15
  class << self; attr_accessor :user, :language, :timeout; end

  Project = StarkCore::Project
  Organization = StarkCore::Organization
  Key = StarkCore::Key

end
