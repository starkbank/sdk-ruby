...
module StarkBank
  ...
  class Workspace < StarkBank::Utils::Resource
    ...
    def self.create(username:, name:, user: nil)
      StarkBank::Utils::Rest.post_single(entity: Workspace.new(username: url, name: subscriptions), user: user, **resource)
    end
    ...
    def self.query(limit: nil, username: nil, ids: nil, user:nil)
      StarkBank::Utils::Rest.get_list(limit: limit, username: username, ids: ids, user: nil, **resource)
    end
